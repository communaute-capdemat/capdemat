...
<html>
  <head>
     <title>${message(code:'ddsr.description')}</title>
  ...
  <body>
    ...
    <div id="request" class="main-box ${rqt.requestType.category.name}">
      <h2>
        ...
        <g:message code="ddsr.label" /> <g:message code="message.number" args="[rqt.id]" />
      </h2>
      <p><g:message code="request.duration.label" /><strong> : <g:message code="ddsr.duration.value" /></strong></p>
      <p>
      ...
      <div class="datas">
         <g:set var="requestTypeAcronym" value="ddsr" scope="request" />
      ...
      <ul>
        ...
        <li class="${currentStep == 'demande' ? 'current ' : ''}
          
            ${individual ? rqt.stepStates['demande-' + params.type].state : rqt.stepStates['demande'].state}
          
          ">
          <span class="number">1</span>
          <a
            <g:if test="${currentStep != 'demande' && rqt.stepStates['demande'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'demande'])}"
            </g:if>
          >
            
              <g:message code="${individual ? 'homeFolder.action.add' + org.apache.commons.lang.StringUtils.capitalize(params.type) : 'ddsr.step.demande.label'}" />
              ${individual ? '' : '*'}
              <span class="help">
                <g:message code="request.step.message.${rqt.stepStates['demande' + (individual ? '-' + params.type : '')].state}" />
              </span>
