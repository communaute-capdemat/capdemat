...
<html>
  <head>
     <title>${message(code:'taror.description')}</title>
  ...
  <body>
    ...
    <div id="request" class="main-box ${rqt.requestType.category.name}">
      <h2>
        ...
        <g:message code="taror.label" /> <g:message code="message.number" args="[rqt.id]" />
      </h2>
      <p><g:message code="request.duration.label" /><strong> : <g:message code="taror.duration.value" /></strong></p>
      <p>
      ...
      <div class="datas">
         <g:set var="requestTypeAcronym" value="taror" scope="request" />
      ...
      <ul>
        ...
        <li class="${currentStep == 'general' ? 'current ' : ''}
          
            ${individual ? rqt.stepStates['general-' + params.type].state : rqt.stepStates['general'].state}
          
          ">
          <span class="number">1</span>
          <a
            <g:if test="${currentStep != 'general' && rqt.stepStates['general'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'general'])}"
            </g:if>
          >
            
              <g:message code="${individual ? 'homeFolder.action.add' + org.apache.commons.lang.StringUtils.capitalize(params.type) : 'taror.step.general.label'}" />
              ${individual ? '' : '*'}
              <span class="help">
                <g:message code="request.step.message.${rqt.stepStates['general' + (individual ? '-' + params.type : '')].state}" />
              </span>
        ...
        <li class="${currentStep == 'technique' ? 'current ' : ''}
          
            ${rqt.stepStates['technique'].state}
          
          ">
          <span class="number">2</span>
          <a
            <g:if test="${currentStep != 'technique' && rqt.stepStates['technique'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'technique'])}"
            </g:if>
          >
            
              <g:message code="taror.step.technique.label" /> *
              <span class="help">
                
                  <g:message code="request.step.message.${rqt.stepStates['technique'].state}" />
        ...
        <li class="${currentStep == 'reglement' ? 'current ' : ''}
          
            ${rqt.stepStates['reglement'].state}
          
          ">
          <span class="number">3</span>
          <a
            <g:if test="${currentStep != 'reglement' && rqt.stepStates['reglement'].state != 'unavailable'}">
              href="${createLink(controller:'frontofficeRequest', action : 'edit', params:['id':rqt.id,'currentStep':'reglement'])}"
            </g:if>
          >
            
              <g:message code="taror.step.reglement.label" /> *
              <span class="help">
                
                  <g:message code="request.step.message.${rqt.stepStates['reglement'].state}" />
